...
pub struct DecoderReader<'a, R: 'a, B> {
    reader: &'a mut R,
...
impl<'a, R: Read, B: ByteOrder> DecoderReader<'a, R, B> {
    pub fn new(r: &'a mut R, size_limit: SizeLimit) -> DecoderReader<'a, R, B> {
        DecoderReader {
            reader: r,
...
impl <'a, A, B> DecoderReader<'a, A, B> {
...
impl<'a, R: Read, B: ByteOrder> Decoder for DecoderReader<'a, R, B> {
    ...
    fn read_f64(&mut self) -> DecodingResult<f64> {
        ...
        self.reader.read_f64::<B>().map_err(wrap_io)
    }
    fn read_f32(&mut self) -> DecodingResult<f32> {
        try!(self.read_type::<f32>());
        self.reader.read_f32::<B>().map_err(wrap_io)
    }
    ...
    fn read_enum<T, F>(&mut self, _: &str, f: F) -> DecodingResult<T> where
        F: FnOnce(&mut DecoderReader<'a, R, B>) -> DecodingResult<T> {
            f(self)
        }
    fn read_enum_variant<T, F>(&mut self, names: &[&str], mut f: F) -> DecodingResult<T> where
        F: FnMut(&mut DecoderReader<'a, R, B>, usize) -> DecodingResult<T> {
    ...
    fn read_option<T, F>(&mut self, mut f: F) -> DecodingResult<T> where
        F: FnMut(&mut DecoderReader<'a, R, B>, bool) -> DecodingResult<T> {
    ...
    fn read_seq<T, F>(&mut self, f: F) -> DecodingResult<T> where
        F: FnOnce(&mut DecoderReader<'a, R, B>, usize) -> DecodingResult<T> {
            let len = try!(self.read_usize());
            f(self, len)
        }
